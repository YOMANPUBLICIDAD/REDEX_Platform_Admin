...
filepath = '/Users/admin/Desktop/REDEX_Premium_Final/index.html'
opt_filepath = '/Users/admin/Desktop/REDEX_Premium_Optimizado/index.html'
...
# Avoid duplicating CSS if we run it twice
if '.detail-modal-overlay {' not in content:
    if '</style>' in content:
...
"""
if '<div id="detail-modal"' not in content:
    content = content.replace('</body>', new_modal_html + '\n</body>')

# 3. Replace JS logic
# Look for window.openProyectoModal block up to window.mSetGallery... 
# wait, index.html might have different JS structure for the old modal.
# Let's use a regex that matches `let mGalleryArr = [];` up to `mSetGallery`.
...
if old_js_pattern.search(content):
    ...
    # If not found, just inject it before closing script, if we can find the openProyectoModal
    pass 
    
# Fix onclick="${p.id}" to onclick="'${p.id}'"
content = content.replace("onclick=\"openProyectoModal(${p.id})\"", "onclick=\"openProyectoModal('${p.id}')\"")
