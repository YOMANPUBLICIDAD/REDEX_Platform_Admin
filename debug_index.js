...
// ============= INMUEBLES DYNAMIC LOGIC =============
function renderInmuebles(filter = 'todos') {
  const grid = document.getElementById('inmuebles-grid');
  if(!grid) return;
  grid.innerHTML = '';
  let filtered = proyectosData;
  if(filter === 'lote' || filter === 'inmueble') {
    filtered = proyectosData.filter(p => p.categoriaFiltro === filter);
  } else if(filter === 'disponible') {
    filtered = proyectosData.filter(p => p.estado !== 'Reservado' && p.estado !== 'Vendido');
  }
  ...
  filtered.forEach((p, idx) => {
    ...
    card.innerHTML = `
      <div class="inm-c-img">
        <img loading="lazy" src="${encodeURI(p.imagenPrincipal)}" alt="${p.nombre}" onerror="this.src='hero_bg.png'">
        <div class="inm-c-tag ${p.estado === 'Disponible' ? 'green' : (p.estado === 'Reservado' || p.estado === 'Vendido' ? 'red' : 'gold')}">${p.estado}</div>
        <div class="inm-c-qty">${p.cantidadDisponible}</div>
      </div>
      <div class="inm-c-body">
        <div class="inm-c-loc">${p.ubicacion}</div>
        <h3 class="inm-c-name">${p.nombre}</h3>
        <div class="inm-c-price">Desde <strong>${p.precio}</strong></div>
        <div class="inm-c-type">${p.categoriaFiltro.toUpperCase()}</div>
        <button class="btn-gold mag" style="width:100%; margin-top:15px; border-radius:4px;" onclick="openProyectoModal('${p.id}')">Ver Proyecto</button>
      </div>
    `;
    ...
    setTimeout(() => {
      ...
      const btn = card.querySelector('.mag');
      if(btn) {
          btn.addEventListener('mousemove', e => {
            const r = btn.getBoundingClientRect();
            const cx=r.left+r.width/2, cy=r.top+r.height/2;
            gsap.to(btn,{x:(e.clientX-cx)*.15,y:(e.clientY-cy)*.15,duration:.4,ease:'power2.out'});
          });
...
function startCarousel() {
  ...
  carouselInterval = setInterval(() => {
    if(isCarouselPaused || isModalOpen) return;
    const grid = document.getElementById('inmuebles-grid');
    ...
    if (grid.scrollLeft + grid.clientWidth >= grid.scrollWidth - 10) {
      ...
      // Avanzar una tarjeta (ancho de tarjeta + gap)
      let cardWidth = grid.firstElementChild.offsetWidth + 30;
      grid.scrollBy({ left: cardWidth, behavior: 'smooth' });
    }
  }, 4000);
}

document.querySelectorAll('#inmuebles-filters .f-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('#inmuebles-filters .f-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    renderInmuebles(btn.getAttribute('data-filter'));
    const grid = document.getElementById('inmuebles-grid');
    if(grid) grid.scrollTo({ left: 0, behavior: 'smooth' });
    startCarousel(); // reiniciar timer
  });
});

if(document.getElementById('inmuebles-grid')){
  renderInmuebles();
  const grid = document.getElementById('inmuebles-grid');
  const wrap = document.querySelector('.inmuebles-carousel-wrap');
  const prevBtn = document.getElementById('inm-prev');
  const nextBtn = document.getElementById('inm-next');
  
  if(prevBtn && nextBtn && grid) {
    prevBtn.addEventListener('click', () => grid.scrollBy({ left: -grid.clientWidth * 0.8, behavior: 'smooth' }));
    nextBtn.addEventListener('click', () => grid.scrollBy({ left: grid.clientWidth * 0.8, behavior: 'smooth' }));
  }
